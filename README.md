...
### Python setup (required for Predict + PDF upload)

The backend runs the ML + PDF-scraping scripts via `child_process.spawn()`, so you must have **Python** installed and the required packages available.

- Check Python: `python --version`
- If your Python command is `py` on Windows, start backend with `PYTHON=py` (env var) so Node uses that executable.
- Install packages (examples):
  - PDF upload scraping: `pip install PyMuPDF`
  - Heart/Diabetes models: `pip install numpy scikit-learn joblib`
  - Breast/Lung models: `pip install tensorflow pillow numpy`

If an endpoint fails with 500, the frontend now shows a more specific message (e.g. missing Python dependency) and logs `details` in the browser console.

### Git LFS setup (required for Lung/Breast .h5 models)

The Lung + Breast predictors use `.h5` model files tracked with **Git LFS**. If you cloned/downloaded without Git LFS, the `.h5` files will be tiny text pointers and the backend will fail with errors like “Unable to open file (file signature not found)”.

Fix:
- Install Git LFS: `git lfs install`
- Download LFS files: `git lfs pull`

If you donâ€™t have Git LFS and just want the backend to run, you can generate small placeholder `.h5` files locally:
- `python Backend/ML/create_h5_models.py --also-write-root-ml`
